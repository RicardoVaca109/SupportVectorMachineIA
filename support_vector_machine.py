...
#Entrenamiento del modelo: Se utiliza el SVC con un kernel lineal para entrenar el modelo y luego se realiza una predicción sobre un nuevo ejemplo (edad 7, salario 700).
...
# Cargando el dataset
dataset = pd.read_csv('horas_estudio.csv')  # Lee el archivo CSV con los datos
X = dataset[['Horas_Estudio','Promedio_Notas']].values
y = dataset['Aprobado'].values 
...
# Predicción de un nuevo resultado (Ejemplo: Horas de Estudio = 9, Promedio = 60.5)
resultado = classifier.predict(sc.transform([[9, 60.5]]))  # Predice si comprará o no
print(f"Predicción para 9 horas de estudio y con un promedio de 60.5: {resultado}")
